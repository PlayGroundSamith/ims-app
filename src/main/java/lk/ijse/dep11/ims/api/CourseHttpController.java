...
public class CourseHttpController {
    ...
    public CourseTo createCourse(@RequestBody CourseTo courseTo) {
        try {
            ...
            PreparedStatement stm = connection.prepareStatement("INSERT INTO course(id, name, duration_in_months) VALUES (?,?)", Statement.RETURN_GENERATED_KEYS);
    ...
    public List<CourseTo> getAllCourses() {
        try(Connection connection = pool.getConnection()){
            ...
            while (rst.next()){
                ...
                String courseName = rst.getString("name");
                int duration_in_months = rst.getInt("duration_in_months");
                taskList.add(new CourseTo(id, courseName, duration_in_months));
            }
            return taskList;
        } catch (SQLException e) {
            throw new RuntimeException(e);
